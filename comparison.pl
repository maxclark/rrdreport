...
foreach (@interface_types) {
  ...
  print "$starttime $endtime\n" if $VERBOSE;
	...
	while (my ($id, $name, $type, $device, $interface, $ccn_rate, $cir, $cir_rate, $overage_rate) = $get_interfaces->fetchrow_array) {
		...
		my ($ninefive, $ninezero, $inbound, $outbound) = calcTransfer($device,$int,$starttime,$endtime);

		printf "%30s: %8s %8s %8s %8s\n", $name, $ninefive, $ninezero, $inbound, $outbound;
	}
...
sub calcTransfer {
  ...
  # It otuputs the 95th & 90th Percentile and GB transfer for the interface.
  # --
	my $device = shift;
	my $interface = shift;
	my $starttime = shift;
  ...
  # --
	$ninefive_in = $stat_in->percentile(95); 
	$ninefive_out = $stat_out->percentile(95);
	$ninezero_in = $stat_in->percentile(90); 
	$ninezero_out = $stat_out->percentile(90);
	
	# For this script, we only care about the higher of the in/out
	# figure out what it is and return the scaled values
	# --
	if ($gb_in >= $gb_out) {
		return (scaleBits($ninefive_in), scaleBits($ninezero_in), scaleBytes($gb_in), scaleBytes($gb_out));
	} else {
		return (scaleBits($ninefive_out), scaleBits($ninezero_out), scaleBytes($gb_in), scaleBytes($gb_out));
	}
}
# ==========
